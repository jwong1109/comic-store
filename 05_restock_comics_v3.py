# Setting a 10 comics limit for restocking per time
# Modifying the change function so it works for both selling and restocking
# comics
...
def change(action):
    ...
    if int_num_comics <= 0:
        messagebox.showerror('ERROR', 'You must enter a number of comics greater than zero!')
        return None

    for comic_details in comics:
        if comic_name == comic_details.name:
            if action == "restock":
                if int_num_comics <= 10:
                    comic_details.num_stock += int_num_comics
                    successful_message(int_num_comics, comic_name, "restocked")
                else:
                    messagebox.showerror('10 Restock MAX',
                                         'You are only allowed to restock a '
